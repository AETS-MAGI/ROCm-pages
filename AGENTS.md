...
## 本GitHub Pagesの位置づけ
これまでの`vega-hbmx-pages`は温存しつつ、ここではより広範なGPUや、ROCmそのものの構造に焦点を当てた航海用ページを作る。つまり、  `vega-hbmx-pages`は主にvega世代からの出発点のまとめであり、一方、このページの役割は`vega-hbmx-pages`の設計思想をより広範のスタックに拡張したものの公開物として機能させることにある。
...
MI25の`llvm-project-private` など非公開情報に触れる場合は、毎回次を明記すること。
...
### 2.4 推奨事項
- 特に関係や関連などを書く場合、言葉による説明だけではなく、Marmaidなどを用い、視覚的な整理と理解を図るようページを作成すること。
- 項目やタイトルと内容が合致しているか確認し、不適切なものは、適切な場所へ移動すること。
...
カードごとに違いはあるものの、文書修正時は、次の順で作業すること。また、具体的なファイル名は、AGENTまたは人間の作業者がファイル編集時にここに追記すること：
